...
@testset "PhaseSlopeIndex.jl" begin
    # tests of data2psi ###############################################
    # two random signals
    signal = [[rand(-1.:0.0001:1., 1000000);] [rand(-1.:0.0001:1., 1000000);]]
    ...
    test_x = [0:0.01:2.;]
    @test all(isapprox.(PhaseSlopeIndex.detrend!(test_x, 0), [-1.:0.01:1.;], atol=0.001))
    test_x = [0:0.01:2.;]
    @test all(isapprox.(PhaseSlopeIndex.detrend!(test_x, 1), zeros_x, atol=0.001))
    ...
    @test hanning_window_12[1] > 0.
    @test hanning_window_13[1] > 0.
